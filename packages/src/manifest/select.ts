 ...
 * `planned` or `out-of-scope`, its OS guard exceeds the device, or its substrate
 * does not match the interactive requirement. For driver nodes, the rung's
 * `target` is matched against `ctx.target` (default `'effect'`).
 *
