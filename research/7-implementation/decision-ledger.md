...
| RT-007 | resolved | `34` | The interruptible-spring retarget contract (no snap, no double-animation). | `FxAnimationDriver` (blueprint Unit 6) | **Resolved (U6-001, 2026-06-12).** The contract implemented and device-verified per platform (POCO F1 / Android 15 API 35 + iPhone 17 Pro simulator / iOS 26.5, maintainer-ratified 2026-06-12): fire-once completes exactly once; mid-flight retarget continues from the presentation value with no snap — iOS logged the render-server → display-link handoff with the captured presentation value, and a second retarget mid-display-link carried live velocity (the opposing-inertia clip observed working on both retargets); cancel settles in place and stops the loop; reduce-motion / animator-scale-0 places in a single frame with one completion. Evidence: `7-implementation/tasks/U6-001/evidence/device-run.md`. RT-016's hard-retarget matrix stays with U6-002. |
...
| RT-013 | resolved | `33` | Content-motion hosted-reachability (strictly `expo-view`?) and New-Architecture-only vs a Paper fallback. Lean: `expo-view` + New Arch only (SDK 56 floor). | — | **Resolved (U5-001, 2026-06-11).** Both halves confirmed on device (iOS 26 sim + Android hardware, maintainer-ratified): the post-layout read works strictly on `FxSurfaceView` (`expo-view` — no hosted-substrate involvement) and only on the New Architecture (Fabric, SDK 56 floor; no Paper fallback — the read point exists only in Fabric's mounting flow). `33` §Open questions resolved; the read mechanic is pinned in `structure.{ios,android}.md` §Layout read; evidence in `7-implementation/tasks/U5-001/evidence/`. |
