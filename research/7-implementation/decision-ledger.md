...
| SPINE-003 | open | `02`/`50` | Where per-effect typed config lives, and how the surface gets its TS types. Lean: canonical in the manifest, generate TS from it. | surface typing, BYO uniforms (`FX-006`) | a config-typing mechanism chosen and wired |
...
| SPINE-013 | open | `02` | **Planned-rung selection policy.** `select()` skips only `out-of-scope`, not `status:'planned'` — so a V1 build could select the Android `content-distort`/`symbol` planned rungs. | the executable V1 manifest | `select()` defines whether `planned` rungs are selectable, build-gated, or excluded from the executable manifest |
