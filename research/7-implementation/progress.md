...
> **Current shipped capability set** lives in
> [`v2-capability-baseline.md`](./v2-capability-baseline.md) — the standing snapshot after the V2
> engine + addenda + the full public surface landed: what ships, what is iOS-only / Android-only,
> what is trigger-gated, and what is deliberately rejected. Publishing (DEF-016) is parked.
...
| DOC-031 | 7-impl | doc-cleanup | merged | no | — | — | — | **MERGED on integration/0.1.x (planner, 2026-06-28; maintainer-directed non-publish V2 closure pass).** Wrote [`v2-capability-baseline.md`](./v2-capability-baseline.md) — the standing post-surface capability snapshot (shipped matrix grounded in the canonical manifest; iOS-only/Android-only peer gaps; trigger-gated build-on-demand list; ratified rejections; publishing parked). Refreshed `HOW-TO-CONTINUE.md` "Where the project is" (was 2026-06-18 / "surface unbuilt, start Unit 10" — now surface complete + baseline pointer + publish-parked) and linked the baseline from this file's top matter. Docs-only; closes no ledger row. **Publishing/DEF-016 untouched (parked).** |
| DOC-032 | wip / 0-spine / 1-surface / 3-motion | ratify + doc-cleanup | merged | no | — | — | — | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-28, this commit).** Lane 1 WIP promotion (planner, 2026-06-28). Promotes the architectural slice of `wip/lane1-signal-grammar.md` and `wip/lane1-declarative-surface.md` into canon without spawning implementation: `04` owns the Lane 1 boundary invariant; `05` owns the native signal grammar and Lane 2 ceiling; `40` owns source-driven signal events and the no-double-envelope rule without freezing the public event prop name; `50` owns the preset-first interaction surface direction; `architecture.md §12` records it as promoted future architecture. WIP docs retained as derivation history with top banners; no DEF row unblocked. [task](./tasks/DOC-032/) |
| DOC-035 | packages / wip | doc-cleanup | merged | no | — | — | — | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-29, this commit).** Stale shipped-code + WIP comment sweep (planner, 2026-06-29; maintainer-flagged before the next WIP). Four nits: (1) `wip/README.md:25` interactive-content-distort cross-ref — already repointed to canon `3-motion/41` d15 under DOC-034 (verified, no-op). (2) `packages/src/surface/types.ts` deleted — a dead placeholder (`export {}` + a TODO claiming `FxView`/`FxPressable`/`FxGroup`/`FxItem`/`Fx` "blank until built"); all ship and carry their own types inline (`surface/index.ts`), zero importers. (3) `packages/src/manifest/manifest.ts:323` symbol section comment "Android planned" → "app-supplied Lottie on Android" (Android symbol ships via app-registered Lottie; DEF-id-free per the Comments Guide). (4) `packages/src/source/types.ts` docblock "V2 ships the one render-server tier" → both tiers (iOS render-server + Android best-effort UI-thread). Gates green: lint clean, `tsc --noEmit` exit 0, build ran, 180/180 Jest. Docs/comments + one dead-file removal; closes no ledger row. |
| DOC-034 | wip / 3-motion / 0-spine / 4-runtime / 5-realization | ratify + doc-cleanup | merged | no | — | — | — | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-29, this commit).** Native animation grammar promotion (planner, 2026-06-29; maintainer-authorized this session). Source-backed the four pending Android rows of `wip/native-animation-api-extraction.md` (`android.animation`, `androidx.dynamicanimation`, Jetpack Compose, Material motion — real citations + a new §Android API extraction) and promoted the **principle only** into canon: `41` decision 15 (the `target → state → clock.phase → clock.keyframes → source` sequence + the hybrid-timing principle), `02` decision 17 (the lowering chain + native primitives private to lowering, per d2), `34 §The spring-axis divergence` (source-backed: Android `dampingRatio+stiffness` across dynamicanimation+Compose vs iOS `duration+bounce`), and `structure.{ios,android}.md §motion` (effect-target rungs mapped to the sequence; `clock.phase`/`clock.keyframes` marked additive future rungs with known native counterparts, not frozen API — iOS `PhaseAnimator`/`KeyframeAnimator`/`UnitCurve` signatures still pending). Maintainer calls honored: principle-only (no `FxTransition`/`FxIosTransition`/`FxAndroidTransition` field names frozen — sketches stay in wip), **no DEF row** (no concrete consumer), no unsourced `MotionScheme` stiffness/damping numbers. WIP retained as derivation history with a promotion banner; `wip/README.md` resolution-plan row marked done. DEF-016/publishing untouched. Docs-only; closes no ledger row. [task](./tasks/DOC-034/) |
| DOC-033 | wip / 7-impl | doc-cleanup | merged | no | — | — | — | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-28, this commit).** WIP resolution plan (planner, 2026-06-28). Records how the four remaining current WIP explorations resolve without starting feature work: promote native animation grammar first after source-backed extraction; promote anchored reveal boundary/surface only when a real anchored reveal appears; keep interactive content-distort parked until a water-ripple consumer; keep native slot layout transitions parked and split reserved-size `FxFlow` from measured-content Boundary L. `wip/README.md` now owns the per-file order; `HOW-TO-CONTINUE.md` and `v2-capability-baseline.md` point at it. No DEF backlog cleanup, publishing, code, or device gate. [task](./tasks/DOC-033/) |
...
device-verified (U13-001, 2026-06-20); `FxView` shipped + device-verified (U12-001, 2026-06-21)** —
the canonical front door, its builder escape hatch, native press feedback, and state-driven content
motion are live. Also merged + device-verified: `effect`-on-`FxView` (U12-002, 2026-06-21), typed
material `tint`/`colorScheme` (U15-001, 2026-06-21), and `FxGroup`/`FxItem` (U14-001, 2026-06-22).
The symbol surface (U10-002 — builder-only `fx.effect.symbol`, no `symbol-*` string id) is **MERGED on
integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-28**; iOS 18.5 parity gate PASS; native
already shipped, this added the JS builder). **With U10-002 the V1 surface contract is complete** — all eight
contract symbols (`fx, FxPresence, FxView, FxPressable, Fx, FxGroup, FxItem, EdgeGlow`) + `fx.effect.*` shipped
and device-verified; no surface unit remains unbuilt.
Decision rows these consume
...
| U10-002 | Unit 10 | implement | merged | yes | — | — | — (U10-001 + U11-001 merged) | **MERGED on integration/0.1.x (this commit; device-verified + reviewed + docs-closed, planner-ratified 2026-06-28; maintainer-delegated merge-tick; maintainer-ran gate). [task](./tasks/U10-002/README.md) · [device.md](./tasks/U10-002/evidence/device.md) · [notes](./tasks/U10-002/notes.md). Fork resolved by the maintainer: BUILDER-ONLY.** `fx.effect.symbol({name, animation, trigger?, replaceWith?})` — sole symbol surface, no `symbol-*` string id; discriminated `EffectStep`; routes to `FxHostedView.symbolConfig`; `resolveEffect` string-only; `name` required (tsc-proven via `config.ts SymbolNameIsRequired`); `name:'heart'` manifest default removed; docs `24`/`55` corrected. **Device gate PASS on iPhone 16 Pro sim / iOS 18.5** (JS Metro reload, no native rebuild): builder column == direct `FxHostedView` column across bounce/pulse/scale/appear/disappear/variableColor + iOS-18 breathe/rotate/wiggle + `star→star.fill` replaceWith + trigger value/state/repeat; 12 screenshots in evidence/. Android/iOS<17 no-op = code-reasoned residual (no hardware; mechanism-redundant). Headless: packages tsc/build/lint/**156 tests**; example tsc; internal-id leak scan clean. Fix-round (4 maintainer findings: dead `@ts-expect-error`→real `config.ts` assert; broken step-type imports→narrowing; `PublicNodeId` reverted; device-gate shapes corrected) applied + re-gated. **Pending: maintainer merge-tick + finishing commit on integration/0.1.x.** `fx.effect.symbol({name, animation, trigger?, replaceWith?})` as the sole symbol surface; **no `symbol-*` string id** (a string id means "draws with no required config"; `name` is the symbol's visual identity, not an optional tweak, and has no truthful default). **Most of the unit already exists** — native `FxSymbolView`/`symbolConfig` Prop/`setSymbolConfig`, the manifest `symbol` node (iOS os17/hosted; Android planned → `{via:'none'}`; lowering ladder unchanged), `SymbolConfig`/`SymbolAnimation` catalog+config types, and the `FxHostedView.tsx` `symbolConfig` binding all shipped (U3-007/U3-008/U2-003). The gap is only the public builder: discriminate `EffectStep` (no `MaterialConfig`/`SymbolConfig` union soup), add `fx.effect.symbol` (+`.symbol()` method, `name` required in TS), route the symbol step to `FxHostedView.symbolConfig` (not `effect`; `resolveEffect` stays string-only — early step branch / `resolveEffectStep`), select the manifest symbol node so Android/iOS<17 degrade through one `{via:'none'}` path; keep symbol terminal. One data reconciliation: remove the `name:'heart'` default (`manifest.ts:319` + `data-layer.md:243`) — contradicts no-truthful-default; type-safe. Docs: correct `24 §58` (zero-config example → aspirational/deferred + the string-vs-builder rule) + `55` V1 scope (symbol terminal builder step now ships). iOS device gate = builder==direct-symbolConfig parity + Android no-op (iOS-17 degradation conditional/residual). |
...
| U12-001 | Unit 12 | implement | merged | yes | — | — | — (Units 4/6/7 + U10-001 merged) | **MERGED on integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-21; maintainer merge-tick delegated; pushed to PR #7).** [task](./tasks/U12-001/) · [device.md](./tasks/U12-001/evidence/device.md). `FxStateView` content host (iOS + Android) + `FxStateViewCoordinator` N-state FSM + `FxView` JS surface; reuses `FxPressableView` wrapper pattern + `FxAnimationDriver` as-is. Settle-only `onFxStateChange` with interrupt semantics. **Device-verified both platforms (gate owner ratified):** visible lift + no clip cutoff, one settle per flip, interrupt ordering (hand-ratified — synthetic can't race the spring), touch-through, no regression. **Law resolved — magnitudes diverged per platform:** iOS `scale 1.03`/`translateY −3pt`, Android `scale 1.04`/`translateY −6dp` (density-scaled; `View.translationY` is px). **Two bounded review/gate fixes folded:** `super.applyResolvedConfig()`; the Android px→dp density-scale + `clipChildren=false` overdraw parity (Android `ViewGroup` clips by default, iOS wrapper does not). Gates re-run by reviewer: tsc/lint/**142 tests**/build/swift-lint; Android `compileDebugKotlin` SUCCESSFUL; iOS `FxStateView` ×8 in `Pods.xcodeproj` (pod install ran); both modules registered; no internal-id leaks. **Docs-closed:** `structure.{ios,android}.md` (state host + driver + dispatch + clip parity); `40` `onStateChange` wired + payload reconciled to `{state,finished,interrupted}`; `57 §FxView` state vocab resolved + seeded lift defaults; `52`/index export; blueprint Unit 12 corrected (native as-built) + effect split to U12-002. Commits `6022aa7` + `b88fd1c`. **Pending maintainer merge-tick + push.** [notes](./tasks/U12-001/notes.md) |
| U12-002 | Unit 12 | implement | merged | yes | — | — | U12-001 | **MERGED on integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-21; maintainer merge-tick delegated). Spawned from the U12-001 scope split (2026-06-20).** The `effect` prop on `FxView` — JS composition only, no new native code. `<View pointerEvents="none" style={absoluteFill}><Fx effect={effect} style={absoluteFill} /></View>` as first child inside `NativeFxStateView` → enters the intermediate container → behind content (earlier child = lower z-order), moves with lift transform, touch passes through. Behind-only V1; on-top overlay deferred. No `interactionMode`, no effect events, no shape clipping. One impl file: `FxView.tsx` (commit `379e62c`). **Reviewed:** gates re-run independently by planner (tsc/lint/142 tests/build + example tsc green); `Fx` style-forward verified; comment iceberg-clean (tightened per maintainer); no fix-round. **Device-verified both platforms (maintainer-ratified 2026-06-21):** effect behind content, lifts with it, touch-through, rapid-flip stability (Row 4 hand-ratified — interrupt FSM is U12-001's, unchanged), no-effect regression; all 11 screenshots verified present. **Docs-closed:** `structure.{ios,android}.md` effect notes flipped (JS-composed behind-content child, not a native prop); `57` Decision 2 + example record behind-content shipped. [task](./tasks/U12-002/) · [device.md](./tasks/U12-002/evidence/device.md) |
| U13-001 | Unit 13 | implement | merged | yes | — | — | — (Unit 8 merged) | **MERGED on integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-20; maintainer merge-tick delegated; pushed to PR #7).** `FxPressable` over the shared `FxPressHandler` FSM (host protocol: `<Fx interactionMode>` + new `FxPressableView` content host). Three review rounds resolved: unregistered views / un-activated recognizer / two effect-surface regressions (round 1); the event pipeline was killed by removing `Events(...)` to dodge `topPress` (round 2 — fixed via `onFx`-prefixed events + JS mapping); the Android black square (null-shape ripple mask → `RectShape` mask) and the small inscribed ripple (→ container pressed-state drive + `RADIUS_AUTO` full-cover, **Option A**); plus a comment-density pass. **Device-verified, full matrix PASS both platforms (maintainer-confirmed):** mount, feedback (iOS scale/opacity, Android full-cover ripple), event order, long-press suppression, cancellation + scroll-yield (`onPressOut` only — the agent-gate FAILs were synthetic-gesture artifacts), rapid re-press, and `<Fx interactionMode>` no-regression. **Gates green both platforms:** lint/build/138 tests/example tsc; iOS `xcodebuild` (after `pod install`); Android `compileDebugKotlin`/`assembleDebug`. **Docs-closed:** `structure.{ios,android}.md` feedback mechanic; `57`/`56` `feedback="native"` device-verified; blueprint Unit 13 Shape note corrected (native unit, not "JS/Surface"). Full record: [reviews/U13-001.md](./reviews/U13-001.md) + [evidence/device.md](./tasks/U13-001/evidence/device.md). [task](./tasks/U13-001/) |
| U14-001 | Unit 14 | implement | merged | yes | — | — | — (Units 1/3 merged) | `FxGroup`/`FxItem` — the glass morph compound. **MERGED on integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-22; maintainer merge-tick delegated). Commits `bf4c372` (headless) + `fa95df1` (Android gate fix).** iOS `FxGroupView`: `UIVisualEffectView` + `UIGlassContainerEffect` (guarded `@available(iOS 26)` + `NSClassFromString`) + symmetric `mountChildComponentView`/`unmountChildComponentView` into `contentView` — mirrors `GlassContainer.swift`. Android passthrough, no morph. `FxItem` is a JS Fragment (zero native layers). **iOS-26 spike PASSED:** sibling glass merges even though nested as a grandchild of `contentView` (under `FxHostedView`) — the carried spike risk did NOT bounce; `FxItem` stays JS-only. **Device-verified both platforms:** iOS 26 morph close / separate apart + touch-through + content ride; iOS <26 individual no-crash; Android individual no-morph no-crash + touch-through. **GATE-FOUND ANDROID DEFECT FIXED:** first-mount collapse — `ExpoView` is a `LinearLayout` and an intermediate `FrameLayout` both re-arrange absolutely-positioned children (the `FxStateView` pattern is single-full-bleed-child only); routed children into a `ReactViewGroup` (no-op `onLayout`) so the Fabric frames survive (`fa95df1`). **Reviewed: all gates re-run independently** (packages tsc/lint/145 tests/build; example tsc; Android `compileDebugKotlin --rerun-tasks` 58-exec; iOS `xcodebuild`); diff read, internal-id audit clean, no fix-round. **Docs-closed:** `structure.{ios,android}.md` (UIKit container + routing + divergence); `57` Decision 4 + wording tighten + stale `GlassEffectContainer`→`UIGlassContainerEffect`; `21` Decision 5 + resolved note; `52`/index export; blueprint Unit 14; research/README compound row. [task](./tasks/U14-001/) · [device.md](./tasks/U14-001/evidence/device.md) |
...
| U15-001 | Unit 15 | implement | merged | yes | — | — | — (Unit 3 merged) | typed **material** config (audit-2026-06-18 §A2 / DOC-025, ratified Phase-S build). **MERGED on integration/0.1.x (device-verified + reviewed + docs-closed 2026-06-21; maintainer merge-tick delegated). Commits 7440f19 + 89b594b + 00f6d6e.** Ship **`tint` + `colorScheme` only; `weight` STRUCK** (no `UIGlassEffect` thickness — fallback-only backing is the `fill`/`filter` trap; overlaps `variant`). Native backing: iOS `effect.tintColor` + `effectView.overrideUserInterfaceStyle`; Android fx-drawn scrim color + light/dark base. Public field is `tint`. Scope: manifest `material.uniforms` + `catalog.ts MaterialConfig` (tsc lockstep `config.ts:80`) + iOS `FxGlassSurfaceView`/`FxMaterialView` + Android material draw + `21`/`02`/data-layer/structure reconciliation. **Reviewed: all gates re-run independently** (packages tsc-lockstep/lint/145 tests/build; example tsc; Android `compileDebugKotlin --rerun-tasks` 58-exec; iOS `xcodebuild` BUILD SUCCEEDED); 3 maintainer-flagged fixes folded (Android tint parser aligned to iOS `#RGB/#RRGGBB/#RRGGBBAA` alpha-last; harness public-path row; `02` `default?`). **Device-verified 6/6 both platforms (maintainer-ran, gate build `89b594b`)** — FxHostedView + public `<Fx effect={fx.effect.glass(…)}>` render identically; no regression. Reviewer relocated stray repo-root `tasks/` evidence to canonical path + corrected device.md commit header. [task](./tasks/U15-001/) · [device.md](./tasks/U15-001/evidence/device.md) |
...
| DEF-025 | 2-effects | implement | merged | yes | SPINE-005 | FX-010 | — (trigger: maintainer picked Android symbol parity 2026-06-28) | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-28, this commit).** **Android `symbol` realization — Lottie via `registerSymbol`, feature-gated rung.** Closes the `symbol` cross-platform peer gap. **DEVICE-VERIFIED (maintainer-ratified 2026-06-28):** Android re-gate all PASS (S1 renders sized, S2 discrete-once/indefinite-loop, S3 missing-asset onError+warn no crash, S4 peer-absent `{via:'none'}` no `NoClassDefFoundError`, off-window pause) + iOS S5 (builder trigger-normalization animates indefinite symbols by default). **Shipped:** JS `registerSymbol`/`isRegisteredSymbol`/`registeredSymbolNames` (`effects/symbolRegistry.ts`, mirrors `registerShader` — JSON crosses once keyed by name, iOS no-op) + index export; builder trigger-normalization (`effects/stack.ts`, discrete→`'value'`/indefinite→`'repeat'`); runtime feature detection (`runtime/capabilities.ts` reads native `Constant("features")` → `ctx.features` in `Fx.tsx` — first runtime consumer of `requires.feature`, SPINE-005); manifest Android symbol rung flipped selectable (`feature:'lottie'`); Android `FxSymbolRegistry.kt` + `FxSymbolView.kt` (`FrameLayout` host, lazy Lottie child behind a `Class.forName` guard, `onLayout` child-measure) + `FxModule.kt` probe/`registerSymbol`/`Prop`; iOS `FxModule.swift` no-op + empty features; `compileOnly` Lottie peer. **Review (planner):** found the wrong-runner test artifact (real: Jest 179/179), the 0×0 layout blocker, then the S4 class-load crash (subclass→`FrameLayout` rework — see `reviews/DEF-025.md` + `notes.md`); fixed the iOS/Android `Constants`→`Constant` deprecation + broadened the Lottie probe to catch `LinkageError`. **Headless:** lint/build/Jest 179-179/iOS `xcodebuild`/Android `compileDebugKotlin` all green. Docs-closed (planner, 2026-06-28): FX-010 minted + FX-009 superseded + SPINE-005 feature-flag half advanced in the ledger; `24` realization recorded; `structure.android.md § symbol` confirmed; manifest reconciled; `reviews/DEF-025.md`. [task](./tasks/DEF-025/) · [device.md](./tasks/DEF-025/evidence/device.md) · [notes](./tasks/DEF-025/notes.md) · [review](./reviews/DEF-025.md). |
| DEF-027 | 1-surface / 4-runtime | implement | merged | yes | — | SURF-011 | — | **MERGED on integration/0.1.x (`f5c3662`, 2026-06-29). DEVICE-VERIFIED (maintainer-ratified) + DOCS-CLOSED + REVIEWED.** `FxReveal` anchored-reveal geometric spine (step 1), device-verified both platforms (iOS 26.5 + Android API 35): collapsed-card tap opens, expanded Shutter takes touch + above tab bar, no-reflow, cross-fade, interruption ordering, background-pause, reduce-motion, and W8 (non-full host + initial open, no wrong-size flash) all PASS. **Docs-closed (planner):** mechanics pinned in `structure.{ios,android}.md § reveal`; `54 §Placement & portal coexistence` (reveal-host requirement) + `50 §Props by default` (anchored-reveal direction) ratified; `41` decision 16 (non-uniform `scaleX`/`scaleY` + `origin` IR channels); `04 §The cross-tree frontier` (FxReveal Boundary-A-by-construction, `FxAnchor` deferred to `05`); manifest `motion.properties` gained `scaleX`/`scaleY`; **ledger SURF-011 minted + resolved**; WIP `anchored-reveal-and-library-shape.md` partly promoted (banner) — chrome/`FxAnchor`/`FxFlow`/Boundary L remain future; `reviews/DEF-027.md`. **Step 2 (chrome: radius morph + clip) is a future DEF row.** Awaiting the finishing commit on integration/0.1.x. ↓ prior trail — **ALL DEVICE GATE ROWS PASS — both platforms (planner-driven, agent-device, 2026-06-29); ready for maintainer `device-verified` tick → docs-closed.** Bounded W8 re-gate after the tightening: on POCO F1 / Android API 35 + iPhone 17 Pro / iOS 26.5, the example "W8: ON" toggle mounts the reveal in a `height:420` non-full-window host with initial `open=true` — the expanded panel renders **bounded within the host** (not window-sized; no wrong-size flash; `expand finished=true`) and the **Shutter is touchable** (Android 0→2, iOS 0→1) after layout. W1–W7 already passed (Android headline fix: Shutter takes touch + above tab bar; iOS regression clean). Evidence: `evidence/device-run-{android,ios}/w8-non-full-host.png` + `reveal-host-shutter-works.png`. The reveal is a real interactive spine on both platforms; the initial-open flash is closed by construction (no window fallback + deferred native visibility). Next: maintainer `device-verified` tick → planner docs-closed (record proven mechanics in `structure.{ios,android}`; finish `50`/`41`/`04`/`05`/manifest ratification; mint ledger row; promote the anchored-reveal WIP; `reviews/DEF-027.md`). ↓ prior trail — **PRE-MERGE TIGHTENING HEADLESS-DONE (2026-06-29)** — JS expanded wrapper is `0×0`/`opacity:0` before `hostSize` resolves (removes `useWindowDimensions` fallback); native coordinators defer `setExpandedInteractive(true)` + `snapExpanded` until `hasResolvedContentSize` on the `open=true` initial seat (both platforms symmetric). W8 harness affordance added to `example/screens/reveal.tsx` (W8 mode toggle → fixed-height 420px non-full-window host + `FxReveal` initial `open=true`). Gates: lint/build/Jest 187/187; example tsc exit 0; Android `:react-native-fx:compileDebugKotlin` + `:app:assembleDebug` BUILD SUCCESSFUL; iOS `xcodebuild` BUILD SUCCEEDED. State stays `device-pending` — maintainer runs bounded W8 re-gate (row W8 in `evidence/headless.md`). ↓ prior trail — **REVEAL-HOST DEVICE GATE PASS — both platforms (planner-driven, agent-device, 2026-06-29); awaiting maintainer device-verified tick + the initial-open residual call.** Re-gate after the app-placed-host rework: **Android (POCO F1 / API 35) — the headline fix lands:** the expanded **Shutter takes touch (counter 0→2)** through the reveal and now sits **above the tab bar** (size-authority fixed); collapsed-card tap opens the camera; expanded content is in the AX tree (was absent). **iOS (iPhone 17 Pro / iOS 26.5) regression clean:** card-tap opens, Shutter increments 0→2 (the iOS fill-host change didn't regress). Gates re-run green: lint, Jest 187/187, tsc, Android `compileDebugKotlin`+`assembleDebug`, iOS `xcodebuild`. Evidence: `evidence/device-run-android/reveal-host-shutter-works.png`, `device-run-ios/reveal-host-collapsed.png`. **One residual to decide before merge:** `FxReveal.tsx` uses window dims as the pre-layout fallback — a non-full-window host mounted with `open=true` could briefly size the expanded subtree from the window before `onLayout` supplies host size (planner recommends tightening: hide/zero the expanded slot + defer native expanded visibility until host size resolves). Next: maintainer device-verified tick + residual call → docs-closed (record proven mechanics in `structure.{ios,android}`; ratify `50`/`41`/`04`/`05`/manifest; mint ledger row; promote the WIP). ↓ prior trail — **ROUND-3 FIX HEADLESS-DONE (executor, 2026-06-29) — host-sized expanded slot (`onLayout`) + evidence row 9 updated; state device-pending.** `NativeFxRevealProps` gains `onLayout`; `FxReveal` tracks host dimensions from `FxRevealView.onLayout` and uses them for the expanded slot size (window dimensions as initial fallback, correct for full-window hosts). Evidence row 9 updated to reflect host-local native placement + JS `onLayout` sizing. Gates: lint/build/Jest 187/187; example tsc; Android `:react-native-fx:compileDebugKotlin` + `:app:assembleDebug` BUILD SUCCESSFUL; iOS `xcodebuild` BUILD SUCCEEDED. ↓ prior trail — **REVEAL-HOST REWORK HEADLESS-DONE (executor, 2026-06-29) — state device-pending; maintainer re-gates.** Round-2 rework (app-placed host; fill-host + read-collapsed-slot-frame, both platforms): iOS `collapsedFrameInShell()` reads first subview of `collapsedContainer` (Fabric-managed slot frame, not shell bounds); `layoutSubviews()` collapsed-children frame override removed. Android `pointerEvents` reverted `AUTO`→`BOX_NONE`; `collapsedFrameInShell()` reads first child frame. JS `FxRevealView` receives `StyleSheet.absoluteFill`; `style` prop repositioned to the collapsed slot wrapper (positions the slot within the host). Example mounts `FxReveal` inside a root-level `absoluteFill pointerEvents="box-none"` overlay; `style` anchors the collapsed card at `{position:'absolute', left:16, right:16, bottom:insets.bottom+16}`. Gates: packages lint/build/Jest 187/187; example tsc exit 0; Android `:react-native-fx:compileDebugKotlin` + `:app:assembleDebug` BUILD SUCCESSFUL; iOS `pod install` + `xcodebuild` BUILD SUCCEEDED. Re-gate rows W1–W6 in `evidence/headless.md`. [notes](./tasks/DEF-027/notes.md) ↓ prior trail — **PLACEMENT-HOST RATIFIED + REVEAL-HOST REWORK SPEC'D (planner, 2026-06-29).** Ratified into canon (maintainer-confirmed): `54 §Placement & portal coexistence` now carries the reveal-host requirement and `50 §Props by default` notes the anchored-reveal direction — **the app owns the bounds-containing placement host (root overlay / app portal / RN `Modal`); fx owns the reveal animation inside it** (DEF-003-compatible, no fx portal; rule #9). Rework spec'd → [android-rework.md](./tasks/DEF-027/android-rework.md) (now cross-platform: the fill-host + read-collapsed-slot-frame model changes both `FxRevealView.{swift,kt}`; Android host reverts `AUTO`→`BOX_NONE` with phase-gated expanded interactivity kept; example mounts the reveal in a root `absoluteFill` box-none host). Supersedes the round-1 Android-only in-place-resize attempt (which fixed the collapsed-card tap but not the expanded Shutter — see trail). Re-gate when reworked: collapsed-card tap opens + expanded Shutter increments + above tab bar, both platforms, prior rows + iOS unregressed. ↓ prior trail — **PAUSED for a placement-host ratification (maintainer, 2026-06-29).** Android touch RE-GATE after the rework (planner-driven, agent-device): the **collapsed-card tap now opens the camera** (the `ReactPointerEventsView`/`AUTO` fix works) + placement removal/closed-state gating clean (gates green, 187/187, iOS untouched) — but the **expanded Shutter (overflow) is still not touchable**: the executor resized `expandedContainer` within a still-collapsed-sized `FxRevealView` (`setMeasuredDimension` echoes the spec; the expanded layer is its child, not a higher overlay), so RN's `TouchTargetHelper` won't descend for out-of-bounds points (expanded content still absent from the AX tree). Maintainer's call: an internal fx overlay would reopen DEF-003 and a visual-only Android panel is not a real interactive spine — so the direction is **"app-placed reveal host, fx-owned reveal animation"** (DEF-003-compatible: the app supplies a bounds-containing host — root overlay / portal / RN `Modal` — and FxReveal animates inside it; fx never portals). Ratify the placement-host decision (`54` DEF-003 extension + `50`/anchored-reveal direction) + adjust the spec before the next rework + re-gate. Evidence: `evidence/device-run-android/rework-expanded.png`. ↓ prior trail — **REWORK HEADLESS-DONE (executor, 2026-06-29) — state stays device-pending.** Android touch rework + public `placement` removal shipped headlessly: `FxRevealView` now implements `ReactPointerEventsView` (`AUTO`) while each layer stays `BOX_NONE`; the expanded child is laid out in host-local target coordinates and excluded from the flattened touch walk while collapsed; Android native sizing reads the laid-out expanded child when available; `FxRevealProps` no longer exposes `placement` and `FxRevealPlacement` is no longer exported; the example no longer passes `placement="bottom-half"`. Gates green: packages lint/build/Jest 187/187; example `bun x tsc --noEmit`; Android `:react-native-fx:compileDebugKotlin` + `:app:assembleDebug`; iOS `pod install` + example `xcodebuild` (iOS source untouched in the rework). Maintainer re-gate owed: Android collapsed-card tap opens, Shutter increments, Shutter above tab bar, prior Android rows unregressed, iOS card-tap + Shutter smoke, and no public `placement` prop. **DEVICE GATE 2026-06-29 (planner-driven, agent-device): iOS PASS / Android FAIL — Android touch rework owed.** **iOS (iPhone 17 Pro / iOS 26.5):** rows 1–7 + 4b PASS — geometry/sharpness, no-reflow (sibling y identical collapsed/expanded), cross-fade, revealed-content touch (Shutter 0→2 AND collapsed-card tap opens the camera), closed-state-not-touchable, interruption (`expand interrupted=true` emitted before the retargeted `collapse` settle), background-pause, reduce-motion (instant, proven under slow-animations); rows 8/9/10 not exercisable (no sub-17 sim; example portrait-locked). **Android (POCO F1 / API 35): CORE TOUCH BUG —** FxReveal delivers no touch to its hosted RN content: the wired collapsed "Ask anything…" `Pressable` (in-bounds) does NOT open the camera, and the expanded Shutter (overflow) does NOT increment; both inert/absent in the AX tree. The non-interactive rows PASS on Android (geometry, no-reflow, cross-fade, interruption ordering, closed-not-touchable, background, reduce-motion). Root cause grounded: `FxRevealView` (host) never implemented `ReactPointerEventsView` (the U8-001 pattern `FxSurfaceView.kt:39` uses), plus the expanded layer's overflow + offset coordinates. **Rework spec'd → [android-rework.md](./tasks/DEF-027/android-rework.md)** (port the U8-001 touch pattern + host the expanded layer higher + the size-authority single-authority fix; iOS untouched, must not regress). Evidence: `evidence/device-run-{ios,android}/`. ↓ prior headless trail — **HEADLESS-DONE (executor, 2026-06-29).** Step-1 geometric spine shipped: non-uniform `scaleX`/`scaleY` + origin/pivot through `FxAnimationVector`/`FxSpring`/both drivers (iOS polar read-back replaced with independent-axis decomposition; uniform-`scale` callers unchanged) + manifest `motion.properties`; new dedicated `FxRevealView.{swift,kt}` shell (two fx-owned layers, geometry + counter-fade drivers, slot-indexed mount/unmount — chosen over overloading `FxSurfaceView` to isolate two-layer routing, flagged for docs-closed) + `FxRevealCoordinator.{swift,kt}` FSM; `FxReveal` surface (no anchor prop) + native binding + module registration; `reveal` example screen + nav; `reveal-conformance.test.ts` (+6, guards channels/driver-drift/registration/no-anchor-prop). Gates green: lint (pinned biome) · build · Jest 186/186 · example tsc · Android `compileDebugKotlin` · iOS `pod install`+`xcodebuild`. Device gate is the maintainer's — runbook in `evidence/headless.md`; unverified claims (sharpness, the overlay-hosting open mechanic, Android overflow touch/clipping, interruption ordering) in `notes.md`. [headless.md](./tasks/DEF-027/evidence/headless.md) · [notes](./tasks/DEF-027/notes.md). **Original spec:** **spec'd (planner, 2026-06-29) — [task](./tasks/DEF-027/).** **`FxReveal` geometric spine (anchored reveal, step 1 of 2).** A new native lifecycle component that reveals an fx-owned shell from its **own** collapsed frame to an expanded target (card-sized chrome → bottom-half panel) via the inverse-transform technique (lay at `toRect`, inverse-transform to the self-read `fromRect`, animate to identity; content cross-fades). **Boundary A by construction** — fx owns both endpoints, reads only its own frame, retains no foreign tree endpoint; the cross-tree frontier (`04` Decision 8) is where `FxAnchor`/foreign-rect lands later (deferred). **API (decided, no anchor prop, no public placement prop):** `open` / `collapsed` / `expanded` / `preset="anchoredMorph"` (owns the bottom-half target) / `transition` (hybrid per `41` d15) / `onTransitionEnd`. **Reuse:** `intermediateContainer` + mount routing, the transform+opacity spring driver + retarget (interruption), `FxLayoutObserver` self-read. **Build:** non-uniform `scaleX`/`scaleY` + origin/pivot in the vector/manifest/drivers (iOS polar read-back fix), an `FxRevealCoordinator`, the overlay-not-Yoga placement. **Out of scope:** no FxAnchor, no foreign-rect reads, no shared-element retention, no radius/clip (step 2), no border/background morph, no layout reflow, no iOS content sampling/distortion, no per-frame JS. **Paired docs-closed:** ratify the narrowed direction in `50`/`41`/`04`/`05`/`structure.{ios,android}`/manifest + mint the ledger row + promote the anchored-reveal WIP to derivation history. Step 2 (chrome: radius morph + clip) is a future row after this spine device-verifies. |
| DEF-028 | 1-surface / 4-runtime | implement | device-pending | yes | DEF-027 | — | — (trigger: maintainer picked FxReveal step 2 2026-06-29) | **DEVICE MOUNT-SMOKE + PARTIAL GATE (planner-driven, agent-device, POCO F1 / API 35, 2026-06-29): construction crash FIXED — FxReveal RUNS on Android for the first time; chrome (radius+clip) visibly works; one real bug remains.** Reinstalled the SpringForce-fix APK; logcat clean (no `Couldn't create view` / NPE). Pressing Open reveals a **bottom-half panel with rounded top corners + clipped content** (`Camera preview` rect ~`{y:1144}` = bottom half) and emits `onTransitionEnd expand finished=true` — a fallback could not produce a rounded-clipped animated reveal with a native transition event, so the view genuinely constructs. **G1 (round) + G2 (clip) + open mechanic visibly pass on Android (first time ever).** **REMAINING REAL BUG — collapsed first-mount position:** the collapsed card mounts at `{x:0, y:240}` (top, in-flow, absolute style not applied) and recovers to `{x:44, y:1682}` (bottom) only after a full relayout (W8 toggle). This reproduces on a CONSTRUCTING view, so it is genuine (the original symptom was real; the prior crash merely masked it). **Correction to the prior correction:** the position bug was real all along — I had the executor revert the `updateViewLayout` child-routing override on the assumption it was a fallback artifact, but that override was never tested against a working view; the recover-on-relayout signature (absolute frame dropped on first commit, applied on relayout) fits a dropped child-layout-application, so the reverted `updateViewLayout` routing is now the **prime fix candidate — to validate on device, not assert.** Next: re-apply the `updateViewLayout` slot-routing (keep the SpringForce fix), rebuild, and re-gate first-mount bounds (must be `y>1000` on first mount); then run the remaining G3 (clipped touch) / G4 (interruption) / G5 (reduce-motion) / G6 (no reflow). iOS still all-pass. ↓ prior — **CONSTRUCTION-CRASH HEADLESS-DONE (executor, 2026-06-29) - Android FxRevealView construction fix is headless-clean; NOT device-verified.** Fixed the constructor NPE by assigning SpringForce().apply { stiffness = SpringForce.STIFFNESS_MEDIUM; dampingRatio = SpringForce.DAMPING_RATIO_MEDIUM_BOUNCY } to each chrome SpringAnimation before configuring it. Removed the unproven child-routing rework from the prior fallback-layout diagnosis; the reveal must first run on device before any real first-mount position issue is judged. Added a static reveal-conformance.test.ts guard for the SpringForce initialization shape. Gates green: bun run lint (52 files, no fixes) · bun run build · bun run test (9 suites, 191/191; existing Expo logger warnings only) · example bun x tsc --noEmit exit 0 · Android :react-native-fx:compileDebugKotlin BUILD SUCCESSFUL · Android :app:assembleDebug BUILD SUCCESSFUL. Device re-gate owed: Android mount-smoke first (no create-view crash / InvocationTargetException / SpringForce NPE; one collapsed card, not stacked fallback) -> first-mount bounds -> full G1-G6; iOS smoke. [headless.md](./tasks/DEF-028/evidence/headless.md) · [notes](./tasks/DEF-028/notes.md). ↓ prior — **CORRECTION (planner, 2026-06-29, 2nd device pass): the real Android root cause is a CONSTRUCTION CRASH, not a layout bug — `FxReveal` has NEVER actually run on Android.** Logcat (POCO F1, agent-device, rework build): `Couldn't create view of type FxRevealView` / `NullPointerException: SpringForce.setStiffness(float) on a null object reference at FxRevealView.<init>(FxRevealView.kt:106)`. `SpringAnimation(FloatValueHolder).spring` is null until a `SpringForce` is set; `FxRevealView.kt:106-107` does `anim.spring.stiffness = …` → NPE in the constructor → Expo falls back to raw stacked slot Views. **Present in the ORIGINAL DEF-028 build** (line 106 unchanged by the rework; only the damping value changed) — so the prior "first-mount card-at-top" symptom + the flattened-walk root cause + the child-routing rework all measured the **crashed view's fallback** (the `y:240`→`y:1682` bounds were fallback artifacts), NOT a working reveal. iOS passes (separate `CASpringAnimation` path). Headless gates miss it (Kotlin platform-type compiles; JS tests never instantiate the view) → a device **mount-smoke** is now a required first Android gate row. **Fix (`FxRevealView.kt:105-107`):** set a `SpringForce` on each chrome anim before its properties. **Re-gate the FULL Android suite from scratch** (mount-smoke → first-mount position → G1–G6); do NOT assume the position bug is real until a constructing view is observed; if the slot is correct after the fix, the child-routing rework may be reverted. Spec → [android-rework.md](./tasks/DEF-028/android-rework.md) (CORRECTION section). Planner self-correction: I measured bounds on the first diagnostic but did not pull logcat — the crash was visible there too. ↓ prior (SUPERSEDED — mis-diagnosed as a layout bug) — **REWORK HEADLESS-DONE (executor, 2026-06-29) — Android first-mount-position fix ready for maintainer re-gate; NOT device-verified.** Android `FxRevealView` child-management facade completed around the new chrome wrapper: `updateViewLayout`, `removeViews`, and `removeViewsInLayout` now route through the real collapsed/expanded slot containers; internal collapsed/chrome children use host layout params; the scale-free chrome boundary stays intact (`chromeContainer` unscaled, `expandedContainer` remains the transformed inner layer). `reveal-conformance.test.ts` now guards these Android routing methods. Gates green: `bun run lint` (52 files, no fixes) · `bun run build` · `bun run test` (9 suites, 191/191; existing Expo logger warnings only) · example `bun x tsc --noEmit` exit 0 · Android `:react-native-fx:compileDebugKotlin` BUILD SUCCESSFUL · Android `:app:assembleDebug` BUILD SUCCESSFUL. Device re-gate owed: fresh first-mount collapsed-card bounds (must be bottom-anchored before W8/full relayout) + full Android G1–G6 + iOS smoke. [headless.md](./tasks/DEF-028/evidence/headless.md) · [notes](./tasks/DEF-028/notes.md). ↓ prior — **DEVICE GATE 2026-06-29: iOS PASS / Android FAIL (G6 step-1 regression) — rework owed.** iOS (iPhone 17 Pro / iOS 26.5): all gates PASS. **Android (POCO F1 / API 35): collapsed slot first-mount position regression** — the collapsed card (tap-to-open) renders at the TOP of the host on first mount, then recovers to the correct bottom anchor after a remount (W8 toggle). Investigation (planner, file:line): JS structure (`FxReveal.tsx:118-123`) + example host positioning unchanged; the collapsed code path is statically unchanged from device-verified step 1 (the coordinator never repositions the collapsed card — Fabric applies its bottom-anchored frame). The regression is an ordering/timing interaction from the DEF-028 view-tree change — inserting `chromeContainer` as a 2nd `MATCH_PARENT` super-child + re-parenting `expandedContainer` into it (`FxRevealView.kt:91-103`) while the flattened `getChildCount`/`getChildAt`/`indexOfChild` (`:414-431`) still expose `collapsedContainer + expandedContainer` (the real super-children are now `collapsedContainer + chromeContainer` — flattened-walk vs real-children mismatch). **On-device bounds diagnostic (planner-driven, agent-device, POCO F1):** first mount = collapsed card rect `{x:0, y:240}` (top, in-flow — absolute style NOT applied); after a W8-toggle relayout = `{x:44, y:1682}` (correct — `left:16dp`, bottom-anchored). So it is a **wrong-frame** bug (Fabric applies the collapsed slot's absolute frame only on a later pass), NOT overdraw/z-order. **Prime suspect:** the flattened child accessors `getChildCount`/`getChildAt`/`indexOfChild` (`FxRevealView.kt:414-431`) still expose `expandedContainer`'s children as `FxRevealView`'s own, but `expandedContainer` is now a GRANDCHILD via the new `chromeContainer` (`:91-103`) — the flattened-walk no longer matches the real tree `[collapsedContainer, chromeContainer]`, perturbing first-commit child-frame application. Rework spec'd → [android-rework.md](./tasks/DEF-028/android-rework.md) (reconcile the walk with the tree; keep the chrome channel scale-free; iOS untouched). Re-gate: first-mount bottom anchor (bounds) + full G1–G6 Android + iOS smoke. Rework does NOT re-architect the chrome channel (the scale-free mechanic is sound; iOS all-pass). ↓ prior — **REVIEWER-VERIFIED (planner, 2026-06-29) — ready for device gate (G1–G6).** Headless review (file:line, gates re-run independently): scale-free constraint verified both platforms (clip on the outer layer, transform on the inner `expandedContainer`); zero new public props + `cornerRadius` IR-only confirmed. **One substantive finding, fixed + re-verified:** Android chrome damping was `DAMPING_RATIO_NO_BOUNCY` while the reveal's geometry driver defaults to `MEDIUM_BOUNCY` (`FxAnimationDriver.kt:107`, `presetSpring()` null) → G1/G4 desync risk; corrected to `MEDIUM_BOUNCY` (`FxRevealView.kt:107`) + comment fixed (iOS already matched, both no-bounce). G1-visibility radii bumped to 20/36 pt-dp (TODO to device-tune). Residual noted: duplicated `shouldReduceMotion()` (cleanup at a 3rd caller). Re-run gates green (lint · Jest 190/190). Next: maintainer device gate → planner docs-closed. ↓ prior — **headless-done (executor, 2026-06-29).** Layer split + chrome channel shipped: `FxRevealChromeContainer` (`ViewOutlineProvider`+`clipToOutline`) on Android; `chromeLayer`+`CAShapeLayer` mask on iOS. Both coordinators call `snapChrome`/`animateChrome` alongside every geometry call. `cornerRadius` added to manifest `motion.properties` (IR-only). `reveal-conformance.test.ts` +3 (IR channel + no-public-leakage + no-new-props). Gates: lint clean (52 files, no fixes) · build clean · Jest 190/190 · example tsc exit 0 · Android `compileDebugKotlin` + `assembleDebug` BUILD SUCCESSFUL · iOS `pod install` + `xcodebuild` BUILD SUCCEEDED. Device gate is the maintainer's — runbook in `evidence/headless.md`; unverified claims in `notes.md`. [headless.md](./tasks/DEF-028/evidence/headless.md) · [notes](./tasks/DEF-028/notes.md). ↓ **spec'd (planner, 2026-06-29) — [task](./tasks/DEF-028/).** **`FxReveal` chrome step 2 — scale-free radius morph + clip.** Extends the DEF-027 spine: `preset="anchoredMorph"` gains a morphing **corner radius + clip** (square/unclipped today → rounded + clipped). **The pivot (maintainer-decided): chrome is a separate scale-free channel** — radius must NOT sit on the inverse-transformed content layer (a `cornerRadius` under non-uniform `scaleX≠scaleY` goes elliptical). The transformed layer becomes the **inner content-motion layer**; a **new outer chrome/clip boundary** (never scaled) animates its rounded-rect **mask/outline path** in host coords (clip rect collapsed→target, radius source→target). iOS `CAShapeLayer` mask / `masksToBounds`; Android `ViewOutlineProvider` + `clipToOutline` + `invalidateOutline()` (no per-frame layout). Today `expandedContainer` is both transform target **and** content host (`FxRevealView.swift:45,197`) → the build splits it. **Scope: radius + clip ONLY** — no border/background (color/style handoff + source-chrome-read pressure → deferred step 3). **API: zero new public props** — preset owns both radii platform-natively, no source-chrome read (stays Boundary-A); chrome channel is IR/coordinator-private (`cornerRadius` in manifest `motion.properties`), never a public `fx.motion` field (mirrors `41` d16). **Out of scope:** no border/background, no source-chrome read, no new props, no FxAnchor/foreign-rect, no Yoga write, no spine re-architecture. **Device gates:** G1 round-under-morph (the distorted-radius bug), G2 no clip overflow, G3 clipped-content touch survives (Android), G4 interruption keeps transform+radius+clip in sync (one completion), G5 reduce-motion snaps together, G6 no step-1 hit-test/fall-through regression. **Paired docs-closed:** new `41` decision (chrome channel IR-only); `structure.{ios,android} § reveal` (the layer split + mask/outline mechanic, flip the deferred lines); manifest `cornerRadius`; `50` (radius+clip preset-owned, zero props); promote the radius+clip slice of the anchored-reveal WIP; mint ledger row; `reviews/DEF-028.md`. Border/background = future step 3. |
| DEF-026 | 3-motion | implement | merged | yes | — | FX-011 | — (trigger: maintainer picked Android fx-owned `source` parity 2026-06-28) | **MERGED on integration/0.1.x (maintainer-authorized 2026-06-28, this commit).** **Android `source`/scroll realization — native `FxScrollView`, best-effort UI-thread `onScrollChanged` offset→opacity/scale over fx-owned tiles.** Closes the fx-owned `source` cross-platform peer gap. **DEVICE-VERIFIED (maintainer-ratified 2026-06-28):** Android API 35 PASS via agent-device (vertical + horizontal axes animate opacity/scale at 60fps/0 dropped frames, off-window resume); waived/not-run residuals: sub-API-33 degradation and iOS regression (no iOS code changed). **Shipped:** Android native scroll container (`FxScrollView.kt`, core `ScrollView`/`HorizontalScrollView` subclasses), shared hosted-effect factory, module registration, Android native binding selection (deleted static `FxScrollView.android.tsx` fallback), manifest Android rung (`via:'native'`, `primitive:'ScrollView'`, `applyVia:'onScrollChanged'`, `clock:'none'`, `requires {os:21, substrate:'hosted'}`), conformance/select tests, example task entry. Lower fidelity than iOS render-server `.scrollTransition` by design; ambient-RN-scroll remains deferred as its own cross-platform capability. **Headless:** packages gates, `bun run test` 180/180, iOS `xcodebuild`, Android `compileDebugKotlin` green. Docs-closed: FX-011 minted; `structure.android.md § source`, `02` Decision 14, `40`, manifest, and `v2-capability-baseline.md` reconciled; `reviews/DEF-026.md`. [task](./tasks/DEF-026/) · [device.md](./tasks/DEF-026/evidence/device.md) · [notes](./tasks/DEF-026/notes.md) · [review](./reviews/DEF-026.md). |
