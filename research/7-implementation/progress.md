...
| DEF-017 | 6-ship | implement | ready-to-merge | no | — | — | trigger: post-V1 (fired) | **ready-to-merge — reviewed + docs-closed (planner, 2026-06-18); human merge pending. [task](./tasks/DEF-017/).** critique F16: an iOS simulator smoke lane in CI — mount every `CURATED_SHADER_ID` via the catalog switch, screenshot, assert **non-blank** (not golden-image equality) — catches the U3-006 blank-on-switch regression class headlessly, no device sessions. Reuses the `ci.yml` `bare-ios` macos-26 + Metal-toolchain build recipe + agent-device as the sim driver. iOS only (Android smoke deferred with the Android-lib-scaffold reason); device stays the feel/touch gate. **Shipped (commit `78176af`):** the `ios-smoke` ci.yml job + `example/scripts/smoke-shader-catalog.ts`. Proven both ways on a booted iPhone 17 Pro sim (Release): green on all 10 ids (variance ~680–5860 vs threshold 120), RED on a reintroduced blank-frame stub (variance 0.0). Wiring fixes vs the real agent-device CLI: deep-link nav (the U3-006 tile is off-screen), `data.node.rect` rect path, top-down-BMP variance, `prepare ios-runner`. **Review:** gates re-run independently green; tree clean (RED stub + testID wrapper confirmed reverted/behavior-neutral); harness + ci.yml read line-by-line; two minor notes fixed (misleading log; intensity-coverage gap documented as deliberate). F16 docs-closed in `critique-2026-06-10.md`. **Residual:** the first GitHub-hosted `macos-26` run is the remaining proof the hosted-VM sim GPU-renders Metal (the lane only triggers on `main`). Detail in `tasks/DEF-017/README.md`. |
