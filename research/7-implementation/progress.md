...
| U13-001 | Unit 13 | implement | spec'd | yes | — | — | — (Unit 8 merged) | **SPEC'D (planner, 2026-06-20; maintainer-aligned Option A).** `FxPressable` — `feedback="native"` press response + `onPressIn`/`onPressOut`/`onPress`/`onLongPress` on *your* wrapped content. **Reframed from the blueprint's "JS component" (stale — that's the public surface only):** the shipped `FxPressHandler` is an **effect-surface** handler (bound to the Metal `FxSurfaceView`, gates on `containsInteractiveShape`, drives shader uniforms/events), so this is a **native unit**. Architecture: **refactor `FxPressHandler`'s proven FSM behind a host protocol** (hit target / begin-change-end-cancel / long-press / shader-only uniforms / content-only feedback+events) serving BOTH `<Fx interactionMode>` and a new lightweight **`FxPressableView`** (extends `FxNativeView`, mounts children into the intermediate-wrapper *pattern* — not the Metal surface). Feedback: iOS press-in scale/opacity (springs back), Android bounded `RippleDrawable`; magnitudes device-tuned. Cancellation = `onPressOut` only. REJECT a second recognizer (gesture arbitration would drift — `30` D5), Android transform feedback in V1, synthetic `onCancel`, any state/`preset`/`motion` props (U12). Native rebuild gate, both platforms, + `<Fx interactionMode>` no-regression. [task](./tasks/U13-001/) |
