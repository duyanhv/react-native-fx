...
| U7-002 | Unit 7 | device-verify | todo | yes | — | MOT-001 | U7-001 (merged) | device: per-platform preset catalog filled, passes law test; ALSO carries from U7-001: the five `35` React-semantics device rows, the first-mount fade-only layout-timing item (contentHeight→0 on the first prop batch), and the harness log-key LogBox fix (`example/screens/presence.tsx`) |
