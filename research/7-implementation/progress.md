...
| U13-001 | Unit 13 | implement | review-rejected | yes | — | — | — (Unit 8 merged) | **REVIEW-REJECTED round 1 (planner, 2026-06-20); FIX ROUND OWED — local commits only, NOT pushed.** Headless gates green (lint/build/138 tests/example tsc; iOS xcodebuild + Android assembleDebug) but the unit **cannot run** — caught at diff review (native press is untested headless). Blockers: **B1** `FxPressableView` registered in neither `FxModule.swift`/`FxModule.kt` → `requireNativeView` fails at runtime both platforms; **B2** `pressHandler` never put into active mode for `FxPressableView` → recognizer never activates → no events/feedback; **B3** iOS effect-surface regression — passive now fires `onPressIn` (`FxSurfaceView.handlePressBegin` dispatches unconditionally; Android guards correctly); **B4** Android effect-surface regression — drag-tilt no longer settles on end/cancel (lost `settleDragTilt`; iOS preserved). Quality: iOS feedback spring/asyncReset rework, Android `updateEffectSurfaceVisibility` intent. Good: host-protocol shape sound, structure docs pinned, harness wired, internal-id discipline clean. Full record: [reviews/U13-001.md](./reviews/U13-001.md). Spec unchanged (it was correct; the implementation diverged). [task](./tasks/U13-001/) |
