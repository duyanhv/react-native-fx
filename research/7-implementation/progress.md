...
motion are live. Still unbuilt: `effect`-on-`FxView` (U12-002), `FxGroup`/`FxItem`
...
| U14-001 | Unit 14 | implement | device-pending | yes | — | — | — (Units 1/3 merged) | `FxGroup`/`FxItem` — the glass morph compound. **Spec'd + built + reviewed (planner, 2026-06-21); spike-first device gate owed.** iOS `FxGroupView` restructured: `UIVisualEffectView` + `UIGlassContainerEffect` (guarded `@available(iOS 26)` + `NSClassFromString` check) + symmetric `mountChildComponentView`/`unmountChildComponentView` pair routing into `contentView` — mirrors `GlassContainer.swift`, corrects the stale "SwiftUI `GlassEffectContainer`" wording in `structure.ios.md`. Android passthrough with no-morph doc. `FxItem` is a JS Fragment (zero native layers). `FxGroup`/`FxItem` exported. **Reviewed: all gates re-run independently** (packages tsc/lint/145 tests/build; example tsc; Android `compileDebugKotlin --rerun-tasks` 58-exec; iOS `xcodebuild` BUILD SUCCEEDED); diff read, evidence at canonical path, internal-id audit clean, no fix-round. **CARRIED SPIKE RISK** (spec-anticipated): glass is a grandchild of `contentView` (nested under `FxHostedView`), not a direct child as in `GlassContainer.swift` — whether `UIGlassContainerEffect` merges nested glass is the iOS-26 spike question; may bounce for an FxItem/composition change. [task](./tasks/U14-001/) |
