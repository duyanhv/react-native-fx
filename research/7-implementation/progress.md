...
- [ ] rules-gated (#1 native loop, #6 peers, #7 no Reanimated/RNGH/JSI in `packages/`, #9)
- [ ] implemented (preflight + spike on the axis-split → native uniform write → settle)
- [ ] commented
- [ ] headless-done
- [ ] device-verified (axis claim while a cross-axis scroller still scrolls; smooth track under JS load; coexistence — human's gate)
- [ ] reviewed
- [ ] docs-closed (`30` G3 + `structure.{ios,android}` + ledger RT-002)
- [ ] merged
