...
| DEF-025 | 2-effects | implement | headless-done | yes | FX-010 | — | FX-009 (supersedes Android deferred stance) | **headless-done (agent, 2026-06-28).** Android `symbol` rung via app-supplied Lottie (`registerSymbol` registry mirroring `registerShader`); `requires.feature:'lottie'` / `ctx.features` mechanism made real for the first time; builder trigger normalization (discrete→`value`, indefinite→`repeat`). Lottie peer `compileOnly` on library, `implementation` on example app. Gates: lint/build/swift:lint/93 tests (21 new) + Android `compileDebugKotlin` BUILD SUCCESSFUL. Device gate pending — five scenarios in `evidence/headless.md`. [task](./tasks/DEF-025/) |
