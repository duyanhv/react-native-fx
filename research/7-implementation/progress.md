...
| U7-003 | Unit 7 | implement | todo | yes | — | MOT-001 | U7-002 (merged) | **spec'd (planner, 2026-06-12)** — [task](./tasks/U7-003/): plumb the Android presence spring token (`DAMPING_RATIO_NO_BOUNCY` at `STIFFNESS_MEDIUM`, per the pinned catalog) from the coordinator to the driver (driver default unchanged; iOS untouched); device: no-overshoot at slowed animator scale + a presence retarget regression + reduce-motion. **MOT-001 closes here**; the sheet/modal rider re-homes to a deferred row |
