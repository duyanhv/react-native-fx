...
| U14-001 | Unit 14 | implement | in-progress | yes | — | — | — (Units 1/3 merged) | `FxGroup`/`FxItem` — the glass morph compound. **Recommendation pass + spec'd (planner, 2026-06-21).** iOS **UIKit `UIGlassContainerEffect`** (mirror `references/.../GlassContainer.swift` — `UIVisualEffectView` + `mountChildComponentView` into `contentView`), reusing the shipped UIKit `FxGlassSurfaceView` (U3-002) + the U4 child-mount pattern — corrects the stale "SwiftUI GlassEffectContainer" wording in `structure.ios`. **`FxItem` minimal JS** (no native item view unless the spike forces it — the highest-risk fact is whether the system morph needs the glass as a direct contentView child). Android passthrough, **no morph** (DOC-006 ratified divergence); **no `spacing`** (V2). **Spike-first device gate** on iOS 26: confirm two items morph through the FxItem wrapper. `57` "real native morphing view" → "contributes a real native glass surface" at docs-closed. [task](./tasks/U14-001/) |
