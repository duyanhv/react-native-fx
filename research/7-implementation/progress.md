...
| U3-009 | Unit 3 | rework | todo | no | — | — | — (Unit 3 merged) | **SPEC'D 2026-06-19 — [`tasks/U3-009/README.md`](./tasks/U3-009/README.md); dispatchable (append `Task: U3-009.` to the cold-start prompt — it gates U10-001 but sorts after it in the table).** Narrow the `fill`/`filter` manifest over-promise to the rendered subset (audit-2026-06-18 §A2 / DOC-025, ratified). Independently verified against `packages/`: `fill` renderers (iOS `FxFillView.swift`, Android `FxHostedView.kt`) read ONLY the surface `intensity` prop and draw a fixed gradient → trim all six `fill.uniforms` (the renderer ignores `kind`/`colors`/`angle`/`width`/`height`/`drift`); `filter` has NO native renderer → mark both lowering rungs `status:'planned'` so `select()`→`{via:'none'}`. Plus narrow `20-fills`/`23-filters` + update the two Jest suites. `device: no` (no rendering change). Two maintainer open questions parked in the spec (declare `intensity` on the node?; trim the over-promising lowering notes?). `fill` colors/angle/kind wire-through is the spawned Phase-S follow-up |
