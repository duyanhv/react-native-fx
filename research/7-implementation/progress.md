...
| U7-002 | Unit 7 | device-verify | todo | yes | — | MOT-001 | U7-001 (merged) | device: per-platform preset catalog filled, passes law test; **spec'd (planner, 2026-06-12)** — [task](./tasks/U7-002/) (five parts: the transient catalog fill law-tested against the real banner/snackbar, the five `35` React-semantics device rows, the first-mount translate fix, the harness log-key fix, the parity check; MOT-001 closes on ratification, its sheet/modal rider stays deferred) |
