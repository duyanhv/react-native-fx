...
| U15-001 | Unit 15 | implement | device-pending | yes | — | — | — (Unit 3 merged) | typed **material** config (audit-2026-06-18 §A2 / DOC-025, ratified Phase-S build). **Spec'd + built + reviewed (planner, 2026-06-21); device gate owed.** Ship **`tint` + `colorScheme` only; `weight` STRUCK** (no `UIGlassEffect` thickness — fallback-only backing is the `fill`/`filter` trap; overlaps `variant`; no alias, no Android-only). Native backing confirmed vs `references/expo/.../GlassView.swift`: iOS `effect.tintColor` + `effectView.overrideUserInterfaceStyle`; Android fx-drawn scrim color + light/dark base. Public field is `tint` (not `tintColor`). Scope: manifest `material.uniforms` (`color`/`enum` already supported — `UniformSpec.default` made optional for the no-default `tint` case) + `catalog.ts MaterialConfig` (lockstep enforced by `MaterialConfigConformsToManifest`, `config.ts:80`) + iOS `FxGlassSurfaceView` + `FxMaterialView` (below-26, colorScheme only) + Android material draw + `21` reconciliation (`tintColor`→`tint`, strike `weight`). **Reviewed: all gates re-run independently by planner** (packages tsc-lockstep/lint/145 tests/build; example tsc; Android `compileDebugKotlin --rerun-tasks` 58-executed; iOS `xcodebuild` BUILD SUCCEEDED); full diff read, internal-id audit clean (`packages/` clean; the one U15 ref is an on-screen harness label), no fix-round; unplanned `types.ts default?` widening accepted (necessary for no-default `tint`). [task](./tasks/U15-001/) · [notes](./tasks/U15-001/notes.md) |
