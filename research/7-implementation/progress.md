...
| U12-002 | Unit 12 | implement | todo | yes | — | — | U12-001 | **SPAWNED from the U12-001 scope split (2026-06-20, maintainer-aligned).** The `effect` prop on `FxView` — effect decoration **attached to wrapped content** (the `57` `FxView` example lists `effect`; U12-001 deferred it). A distinct ownership problem from state motion: z-order, clipping, hit-testing, composition position, pointer pass-through, and whether the effect is `hosted` or `expo-view` relative to the content host. Owns its own spec + device matrix; closes the `57` `FxView` `effect` line. NOT YET SPEC'D. |
