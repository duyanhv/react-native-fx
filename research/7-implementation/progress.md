...
| U13-001 | Unit 13 | implement | headless-done | yes | — | — | — (Unit 8 merged) | **HEADLESS-DONE after 2 review rounds + a planner fix (2026-06-20); DEVICE FEEL-GATE OWED — local commits only, NOT pushed.** `FxPressable` over the shared `FxPressHandler` FSM (host protocol: `<Fx interactionMode>` + new `FxPressableView` content host). Round-1 reject (unregistered views, un-activated recognizer, two effect-surface regressions) + round-2 issues (the event pipeline was killed by removing `Events(...)` to dodge `topPress`; Android ripple radius locked to 1px; iOS press-down jump) all resolved. **Planner fix `7e3e036`:** native events `onFx`-prefixed + re-declared via `Events(...)`, public `onPress*` mapped in `FxPressable.tsx` (mirrors `onShaderPress*`); ripple radius → `onSizeChanged`; iOS press-down reads current presentation. **Gates green both platforms:** packages lint/build/138 tests/example tsc; iOS `xcodebuild` BUILD SUCCEEDED (FxPressableView genuinely compiled — needed `pod install`); Android `compileDebugKotlin` + `assembleDebug` BUILD SUCCESSFUL. **Device gates so far:** event order `onPressIn→onPressOut→onPress` PASS both platforms (proves the event pipeline fires at runtime — the load-bearing risk); iOS feedback/long-press/rapid-press PASS. **Android FEEDBACK: a black square in the round-1 (pre-fix) capture; STILL UNVERIFIED.** The round-2 re-gate's "still black, fix ineffective" is NOT trustworthy — it cited screenshots that do not exist and the only Android images in evidence are the pre-fix round-1 captures (no post-fix Android screenshot). The original "RippleDrawable content layer" root cause was wrong (a default `ShapeDrawable()` has a null shape, draws nothing). Current hypothesis: the null-shape mask is invalid → the ripple highlight fills the foreground on MIUI. **Mask fix `7921706`** (`ShapeDrawable(RectShape())`, opaque white, null content; assembleDebug green) — strictly more correct, NOT yet device-confirmed. **Owed:** a clean Android re-capture with real present screenshots (at rest + pressed) at the post-fix commit + an isolate-foreground fallback if still black; complete G no-regression; re-capture iOS B/D/F (their screenshots are also missing). Full record: [reviews/U13-001.md](./reviews/U13-001.md) + [evidence/device.md](./tasks/U13-001/evidence/device.md). [task](./tasks/U13-001/) |
