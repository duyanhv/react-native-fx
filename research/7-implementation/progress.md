...
| U6-003 | Unit 6 | ratify | todo | no | — | REAL-001 | U6-001 (merged) | **re-scoped + spec'd (planner, 2026-06-12)** — [task](./tasks/U6-003/): pin the concrete M3 Expressive floor (artifact + version + feature-detection) in `structure.android.md` + reconcile `02`; the standard-spring fallback half of REAL-001 closes by citation (U6-001/U6-002 ran stock `SpringForce` on M3-less MIUI hardware). MOT-002 dropped from this row per DOC-019 (`tune` deferred; resurrects with MOT-001) |
| U7-001 | Unit 7 | implement | todo | yes | MOT-001 | — | U6-001, DOC-018 | device: presence FSM + deferred-unmount handshake; **spec'd (planner, 2026-06-12)** — [task](./tasks/U7-001/); preflight done — see [preflight](./tasks/U7-001/preflight.md) (verdict: sound — JS mount retention validated without the commit hook; the `35` React-semantics rows resolved to FSM rules) |
