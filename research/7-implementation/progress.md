...
| DOC-026 | 0-spine | ratify | todo | no | — | — | — | promote `wip/capability-boundary-classifier.md` (A/B/L boundary taxonomy + substrate-depth axis) into canon `04`/`05` (audit-2026-06-18 §E) — `05 §Decision 5` already cites it as canonical while it is non-authoritative WIP; ratify, then retire/historical the WIP |
| DOC-027 | 7-impl | doc-cleanup | todo | no | — | — | — | record-as-built addendum (audit-2026-06-18 §A2 / DOC-025) — extend `blueprint.md` Phase A / `architecture.md §11` with the smaller shipped-but-unrecorded mechanics: `fx.motion.*` builders (`src/motion/builders.ts`, U7-001), reduce-motion driver gating (`FxAnimationDriver`, incl. the Android `areAnimatorsEnabled()` manual gate), idempotent teardown order (`31 §Idempotent teardown`) |
| DOC-028 | 7-impl | doc-cleanup | todo | no | — | — | — | reconcile `architecture.md §1` target tree (audit-2026-06-18 / the DOC-024 §11 drift note): distinguish **shipped** (`src/source/`, `surface/FxScroll.tsx`, `effects/registry.ts`, `runtime/FxScrollView.*`, `fx.ts`), **Phase-S target** (`surface/{FxView,FxPressable,Fx,FxItem}.tsx`; `presets/{defaults,palettes,themes}.ts`), and **deferred/unbuilt** files |
| DOC-029 | 1-surface | doc-cleanup | todo | no | — | — | — | source-doc narrowing (audit-2026-06-18 §A2 / DOC-025): narrow `50 §Presets/palettes/themes` (no V1 consumer — not a design-token layer) and `55 §EffectStep.node` (drop `fill`/`symbol` terminal steps until render targets + examples exist) so they stop over-promising unbuilt surfaces |
| DOC-030 | wip | doc-cleanup | todo | no | — | — | — | WIP cleanup (audit-2026-06-18 §E): retire `interactive-glass-touch-delivery.md` (ratified + folded into `structure.ios.md`/`01`) and `critique-2026-06-10.md` (its live typed-config/palette thread now lives in U15-001/U3-009/DOC-029); update `wip/README.md` |
...
| U3-009 | Unit 3 | rework | todo | no | — | — | — (Unit 3 merged) | narrow the `fill`/`filter` manifest over-promise to the rendered subset (audit-2026-06-18 §A2 / DOC-025, ratified). `filter` has no native renderer → mark its lowering `status:'planned'`; `fill` renders a fixed gradient → trim the node's `colors`/`angle`/`kind` to the intensity-driven subset. Touches `src/manifest/manifest.ts` + `20-fills`/`23-filters`. **Pre-Unit-10, gates U10-001** — `<Fx effect>` must not offer a rung whose typed config the renderer ignores. `fill` colors/angle/kind wire-through is the spawned Phase-S follow-up |
