...
| U12-001 | Unit 12 | implement | headless-done | yes | — | — | — (Units 4/6/7 + U10-001 merged) | **headless-done (agent, 2026-06-21)** — [task](./tasks/U12-001/). `FxStateView` content host (iOS + Android) + `FxStateViewCoordinator` N-state FSM + `FxView` JS surface. Reuses `FxPressableView` wrapper pattern (intermediateContainer, mount/unmount overrides) + `FxAnimationDriver` as-is. Settle-only `onFxStateChange` with interrupt semantics (`interrupted:true` fires before retargeted settle). `lift` preset (`idle`→identity, `selected`→scale 1.03 + translationY −3). `StateMotionWire` array wire (no dynamic-key Expo Records). `shouldUseAndroidLayout=true` on Android. Registered in both `FxModule.swift` + `FxModule.kt`. JS: `FxView`, `NativeFxStateView`, `toStateMotionWire`. Gates: `bun install`; packages `tsc --noEmit` (clean); `build` (clean); `lint` (Biome clean); `swift:lint` (clean); **142 tests** (4 new `toStateMotionWire` tests); iOS `pod install` + `xcodebuild` **BUILD SUCCEEDED** (`FxStateView.swift` + `FxStateViewCoordinator.swift` compiled); Android `:react-native-fx:compileDebugKotlin` **BUILD SUCCESSFUL** + `:app:assembleDebug` **BUILD SUCCESSFUL**. Mechanics pinned in `structure.{ios,android}.md`. Example harness: `fx-view.tsx` + U12-001 entry in `tasks.ts`. Device gate: state-flip easing + settle event + interrupt ordering + touch-through, iOS sim + POCO F1. [notes](./tasks/U12-001/notes.md) |
