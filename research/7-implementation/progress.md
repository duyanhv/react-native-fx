...
| U8-001 | Unit 8 | implement | todo | yes | RT-005 | RT-006 | U1-002, U3-001, DOC-011 | **spec'd + preflight done (planner, 2026-06-12)** — [task](./tasks/U8-001/): `FxPressHandler.{swift,kt}` FSM both platforms behind `interactionMode` passive/active; slop self-failure as the scroller yield (iOS via the stock-recognizer `isEnabled` toggle — preflight correction); handler-owned long-press timers (platform tokens) + the `onShaderLongPress` dispatcher (`40`); SDF hit-test per `32` D4–5 (UV space); Android adds the setFloatUniform + invalidate-on-write redraw hook. Preflight verdict SOUND ([preflight](./tasks/U8-001/preflight.md)); deltas folded into structure.* §Touch contract. Six-point device gate (incl. one plain-ScrollView yield row + rapid-touch cost → closes RT-006); the full coexistence matrix stays U8-002. **Consumes/closes re-wired with DOC-011 (planner, 2026-06-12):** this task consumes the `32` D4–5 pin and its device gate runs RT-006's tuning + cost rows |
| U8-002 | Unit 8 | device-verify | todo | yes | — | RT-001 | U8-001 | device: cancel path + full RNGH coexistence matrix |
| U9-001 | Unit 9 | implement | todo | yes | — | RT-008 | U6-001, U7-001 | device: `Fx*` SharedObjects wired. **Closes RT-008** (the object granularity finalizes at implementation per its close condition — re-wired from DOC-011, planner 2026-06-12) |
