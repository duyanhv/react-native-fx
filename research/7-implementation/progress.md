...
| U6-001 | Unit 6 | implement | headless-done | yes | RT-007 | RT-007 | U4-001, U5-001 | `FxAnimationDriver.swift`/`.kt` + `FxSpring.swift` owned by `FxSurfaceView`; AndroidX dynamic/core deps declared; headless + iOS xcodebuild + Android assembleDebug green; device scenario written at `tasks/U6-001/evidence/headless.md`; RT-007 closes only on the device gate; U6-002 owns RT-016's device truth |
