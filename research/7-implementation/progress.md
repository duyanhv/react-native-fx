...
| U15-001 | Unit 15 | implement | in-progress | yes | — | — | — (Unit 3 merged) | typed **material** config (audit-2026-06-18 §A2 / DOC-025, ratified Phase-S build). **Recommendation pass + spec'd (planner, 2026-06-21).** Ship **`tint` + `colorScheme` only; `weight` STRUCK** (no `UIGlassEffect` thickness — fallback-only backing is the `fill`/`filter` trap; overlaps `variant`; no alias, no Android-only). Native backing confirmed vs `references/expo/.../GlassView.swift`: iOS `effect.tintColor` + `effectView.overrideUserInterfaceStyle`; Android fx-drawn scrim color + light/dark base. Public field is `tint` (not `tintColor`). Scope: manifest `material.uniforms` (`color`/`enum` already supported — no schema widening) + `catalog.ts MaterialConfig` (lockstep enforced by `MaterialConfigConformsToManifest`, `config.ts:80`) + iOS `FxGlassSurfaceView` + Android material draw + `21` reconciliation (`tintColor`→`tint`, strike `weight`). Native unit; 2-field device matrix both platforms. [task](./tasks/U15-001/) |
