...
| U7-003 | Unit 7 | implement | headless-done | yes | — | MOT-001 | U7-002 (merged) | **headless-done (agent, 2026-06-12)** — [task](./tasks/U7-003/): Android transient presence now passes `DAMPING_RATIO_NO_BOUNCY` at `STIFFNESS_MEDIUM` from the coordinator into `FxAnimationDriver` per envelope; driver no-params default unchanged; iOS untouched. Gates green (`bunx tsc --noEmit`, `bun run build`, `bun run lint`, `bun run swift:lint`, `bun run test` 58 pass, `:react-native-fx:compileDebugKotlin` BUILD SUCCESSFUL). Device handoff: [evidence/headless.md](./tasks/U7-003/evidence/headless.md) — no-overshoot via per-frame logcat capture + a default-spring positive control (review correction: `dynamicanimation:1.0.0` springs ignore the animator duration scale, so slowed-scale capture is invalid) + presence retarget regression + reduce-motion. Reviewed at headless-done (planner, 2026-06-12): scope exact (3 Kotlin files), default path value-identical + self-resetting, gates re-run green. **MOT-001 closes after device PASS + docs closure**; the sheet/modal rider re-homes to a deferred row |
