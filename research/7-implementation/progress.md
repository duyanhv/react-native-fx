...
| U12-002 | Unit 12 | implement | ready-to-merge | yes | — | — | U12-001 | **SPAWNED from the U12-001 scope split (2026-06-20). Spec'd + built + reviewed + device-verified + docs-closed (planner, 2026-06-21); finishing commit owed.** The `effect` prop on `FxView` — JS composition only, no new native code. `<View pointerEvents="none" style={absoluteFill}><Fx effect={effect} style={absoluteFill} /></View>` as first child inside `NativeFxStateView` → enters the intermediate container → behind content (earlier child = lower z-order), moves with lift transform, touch passes through. Behind-only V1; on-top overlay deferred. No `interactionMode`, no effect events, no shape clipping. One impl file: `FxView.tsx` (commit `379e62c`). **Reviewed:** gates re-run independently by planner (tsc/lint/142 tests/build + example tsc green); `Fx` style-forward verified; comment iceberg-clean (tightened per maintainer); no fix-round. **Device-verified both platforms (maintainer-ratified 2026-06-21):** effect behind content, lifts with it, touch-through, rapid-flip stability (Row 4 hand-ratified — interrupt FSM is U12-001's, unchanged), no-effect regression; all 11 screenshots verified present. **Docs-closed:** `structure.{ios,android}.md` effect notes flipped (JS-composed behind-content child, not a native prop); `57` Decision 2 + example record behind-content shipped. [task](./tasks/U12-002/) · [device.md](./tasks/U12-002/evidence/device.md) |
