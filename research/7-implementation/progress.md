...
| DOC-016 | guides | doc-cleanup | todo | no | — | — | — | critique F19: a short human-contributor path in the Contributing Guide (the agent/guide system is the de-facto contributor doc) |
| DOC-017 | 0-spine | rework | todo | no | — | — | — | critique F4: reconcile `01`/`02` worked examples + `research/README.md` substrate wording to the shipped Android hosted mechanic (plain `View` + `RuntimeShader` + `Choreographer`; Compose as a future rung) — the spine currently teaches a mechanic the code rejected |
| DOC-018 | 0-spine/3-motion | ratify | todo | no | — | — | — | critique F5+F12: document presence's scope ceiling ("animates conditional rendering inside a mounted screen; not navigator transitions"); decide the `sheet`/`modal` naming against it; add the React-semantics rows (StrictMode, Fast Refresh, Suspense, mid-exit re-render, list eviction) to `35` — blocks U7-001 |
| DOC-019 | 1-surface | ratify | todo | no | — | — | — | critique F8: defer `tune` from the V1 surface (`preset`/`motion`/`transition` triad ships); MOT-001 resurrects it with device-proven semantics |
| DOC-020 | 1-surface | ratify | todo | no | — | — | — | critique F18: pin the native↔public event-name mapping (`onShaderPress*`/`onFx*` → `onPress*`/`onTransitionEnd`) in one place before any surface component ships |
...
| U3-008 | Unit 3 | rework | headless-done | yes | — | — | — | critique F1+F10: persistent `UIHostingController` + observed props holder on iOS `FxHostedView` (Expo `SwiftUIHostingView` idiom; the Android sibling and the UIKit glass path already update in place) — unblocks the eased-uniform `transition` channel, symbol state survives prop changes; decorative hosted views default a11y-hidden on both platforms; a11y row added to the Device Verification Guide template. Headless gates + xcodebuild green; agent-device evidence (stills only) in `tasks/U3-008/evidence/device-run-2026-06-10/` — F1 symbol/shader continuity PASS, glass regular/clear + GPU resume PASS, decorative a11y-hidden PASS, interactive-glass reachability PARTIAL (no AX element in either state — the open VoiceOver item, see notes); device-verified is the human gate ([task](./tasks/U3-008/)) |
...
| DEF-015 | 1-surface | ratify | blocked | no | — | — | trigger: surface freeze (Unit 5/6 design) | critique F17 (maintainer-accepted 2026-06-10): one naming-review pass before the surface freezes — `<Fx effect={fx.effect.*}>` stutter, `interactionMode` runtime vocabulary, `hosted`/`expo-view` mechanism leakage |
| DEF-016 | 6-ship | implement | blocked | no | — | — | trigger: pre-publish | critique F13: coexistence matrix (RNGH, Reanimated-alongside, navigation, Expo Go/web) + per-capability parity/degradation story + the platform-exclusives positioning in `skills/` |
