# Effect adapter vs compiler (ADR)
Status: researched
...
**Scope: this ADR is about the *effect* lowering only** (render-targets / shaders — the
rungs that emit `.metal`/`.agsl` or bind a native primitive). **Motion is out of scope.**
A spring is not compiled: the `motion` driver is **runtime orchestration over native
animators** (`34`) — there is no shader to emit and no codegen step. The adapter dispatches
the `motion` node's rung like any other; the compiler simply has nothing to do for it.
...
- **Best done as build-time codegen** — a TS tool that reads the manifest/IR and
  ...
  Expo-Modules / no-runtime-C++ decision (no JSI/HybridObject; `05`, the native boundary
  decision). A runtime cross-platform core is only warranted if live IR evaluation
  becomes a real need.
