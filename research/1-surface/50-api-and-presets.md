...
8. **Surface-freeze naming pass ratified (DEF-015, 2026-06-13).** The public surface is frozen with four calls:
   ...
   - **The package publishes as `@react-native-fx/core`,** preserving the `react-native-fx` product identity and every API symbol (`Fx`, `FxPresence`, `FxView`, `FxPressable`, `EdgeGlow`, `fx.effect.*`/`fx.motion.*`). The unscoped `react-native-fx` is unclaimable (npm typosquat filter vs `react-native-fs`); the scope keeps the exact identity. The mechanical `package.json` + docs rename is DEF-016, gated on the maintainer claiming the npm `@react-native-fx` scope first.
