...
export interface CapabilityNode {
  ...
  phase: Phase;
  uniforms?: Record<string, UniformSpec>;  // semantic typed inputs (per-shader detail may defer to its capability doc)
...
```
select(node, platform, ctx = { deviceOS, wantInteractive }):
...
shader: {
  ...
  lower: {
    ...
    android: [
      ...
      { via: 'shader', asset: 'agsl', applyVia: 'graphicsLayer.renderEffect', clock: 'frame-nanos',
        requires: { os: 33, substrate: 'expo-view' }, note: 'draw-time, touch-safe' },
    ],
  },
},
```
  ...
  lower: {
    ios: [
      { via: 'native', primitive: '.layerEffect', requires: { os: 17, substrate: 'hosted' },
        status: 'out-of-scope', note: 'hosting RN content to sample it severs RN touch (05)' },
    ],
    android: [
      { via: 'shader', asset: 'agsl', applyVia: 'RenderEffect', clock: 'frame-nanos',
...
| **Substrate selection** | runtime | `node.interaction`, `requires.substrate` | hosted vs expo-view mount |
...
- **Where does per-shader uniform typing live** — fully inline in `uniforms`, or a
  reference into the capability doc / a generated types file? Lean: canonical in the
  manifest, TS types generated from it.
...
- **`via:'lib'` dependency contract** — a `lib` rung (Haze, Lottie) implies an
  optional peer dependency; decide whether the manifest names the package + version.
...
- `05`/`30-interaction` — why interaction forces the expo-view substrate.
