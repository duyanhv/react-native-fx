...
RN context is a **shaped, host-safe, interactive surface** — a glow-shaped button
that is only touchable *inside the glow*, that yields correctly to scrollers, and
that doesn't sever RN touch. This doc owns that model. It exists **only on the
...
- **Shaped hit-testing (SDF).** For a surface whose *visible* effect is a shape (a
  glow blob, a rounded glass pill, a non-rectangular button), touches outside the
  shape should pass through. The surface evaluates a **signed-distance field** for the
  effect's shape at the touch point and only claims the gesture when the point is
  inside (with a tunable feather). This is what makes a magnetic/glow button feel like
  a real object rather than an invisible rectangle.
