...
Feeds: 02-capability-ir-and-lowering.md, structure.ios.md, structure.android.md, 30–32

## Why this matters

The single most decision-shaping fact in fx is *which native substrate an effect
runs on*. It gates whether the effect can be interactive, whether the runtime (G)
applies, how layout and hit-testing work, and how it composes with RN content. Every
...
- **`expo-view`** — a plain `ExpoView` (a `UIView` on iOS, a `View`/`ViewGroup` on
  Android), hosting the actual render surface. A first-class native citizen:
  recognizers/touch dispatch attach directly and scroll/RNGH arbitration mediates for
  free. **Carries the runtime (G).** Required for any fx-managed interaction.

**The rule:** decorative/generative ⇒ `hosted`; fx-managed interaction ⇒
`expo-view`. A node's `interaction` field (`none | self | fx`, `02`) decides:
`none`/`self` stay `hosted`, `fx` routes to `expo-view` when used interactively.
...
A plain native view keeps the surface in the RN/native view tree, so the runtime (G)
— cooperative gesture recognizer, host-safe and shaped/SDF hit-testing, springs,
frame scheduling — can exist. This is the only place fx-managed interaction works.
Mechanics in `30`–`32` and `structure.{ios,android}`.
...
1. **Two substrates, chosen per capability and per intended use.** `hosted` for
   decorative/self-gesturing (main path); `expo-view` for fx-managed interaction.
